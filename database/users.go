...
func (usersDB *usersDB) GetByEmail(ctx context.Context, email string) (users.User, error) {
	...
	row := usersDB.conn.QueryRowContext(ctx, "SELECT id, email, password_hash, nick_name, first_name, last_name, wallet_address, nonce, last_login, status, created_at FROM users WHERE email_normalized=$1", emailNormalized)
...
func (usersDB *usersDB) GetByWalletAddress(ctx context.Context, walletAddress evmsignature.Address) (users.User, error) {
	...
	row := usersDB.conn.QueryRowContext(ctx, "SELECT id, email, password_hash, nick_name, first_name, last_name, wallet_address, nonce, last_login, status, created_at FROM users WHERE wallet_address = $1", walletAddress)
