...
KeyStorageHandler.prototype.generateKey = function (type) {
    if (type !== "jwt") {
        return this.subtle.generateKey(encryptionTypes[0], false, ["encrypt", "decrypt"]);
    }

    return this.subtle.generateKey(encryptionTypes[1], false, ["sign", "verify"]);
};
...
KeyStorageHandler.prototype.signWithKey = async function (base64PubKey, payload) {
    ...
    if (responce.type === "jwt") {
        return this.subtle.sign(signatureType, responce.keys.webCryptoKeys.privateKey, payload);
    } else {
        let encryptedSecret = bs58.decode(responce.keys.encryptedSecret);
...
KeyStorageHandler.prototype.uploadKey = async function (params = {}) {
    ...
    if (type !== "jwt") {
        ...
        const secretKey =
            typeof operationalKeys.secretKey === "string"
                ? bs58.decode(operationalKeys.secretKey)
                : operationalKeys.secretKey;
        const encrypted = await window.crypto.subtle.encrypt(encryptionTypes[0], webCryptoKeys, secretKey);

        keydata.encryptedSecret = bs58.encode(new Uint8Array(encrypted));
    }

    const base64PubKey =
        type === "jwt" ? this.jwkToBase64PubKey(await this.keydataToJWK(webCryptoKeys)) : keydata.publicKey;
