...
var Error = errs.Class("nft signer cli error")

// Config contains configurable values for card generator project.
type Config struct {
	Database         string `json:"database"`
	nftsigner.Config `json:"config"`
}
...
var (
	...
	rootCmd = &cobra.Command{
		Use:   "signer",
		Short: "cli for interacting with nft signer project",
	}

	// nft signer setup cmd.
...
func cmdRun(cmd *cobra.Command, args []string) (err error) {
	...
	if err != nil {
		log.Error("Error starting nftsigner bank service", Error.Wrap(err))
		return Error.Wrap(err)
	}
